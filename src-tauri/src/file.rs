...
pub fn get_files_name(dir_path: PathBuf) -> Result<Vec<String>, String> {
...
pub fn get_files_path_deep(dir_path: PathBuf) -> Result<Vec<String>, String> {
            ...
            for entry in entries {
                if let Ok(entry) = entry {
                    ...
                    if let Some(file_name) = path.file_name().and_then(|name| name.to_str()) {
                        if !file_name.starts_with('.') {
                            if path.is_dir() {
                                if let Ok(files) = get_files_path_deep(path) {
